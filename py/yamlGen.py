...
for i in range(1,number_node+1):
    ...
    volumes=[]
    volumes.append('./keys/password:/root/files/password:ro')
    volumes.append('./keys/priv.key:/root/files/priv.key:ro')
    volumes.append('./data:/root/data:rw')
    ports=[]
    ...
    stream = open('nodes/node_'+str(i)+'/docker-compose.yaml', 'w+')
